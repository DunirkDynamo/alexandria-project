...
class UniformityPlotter:
    ...
    def plot(self):
        ...
        for label, coord in centers.items():
            ...
            half = size / 2
            roi_data = img[
                int(cy_roi - half) : int(cy_roi + half),
                int(cx_roi - half) : int(cx_roi + half),
            ].flatten()
            roi_datas.append(roi_data)
            ax_hist.hist(
                roi_data, histtype="step", color=color, linewidth=3, label=label
            )
        ...
        for label, mean_val, sem in zip(labels, means, sems):
            std_val = self.results[label.lower()]["std"]
            table_data.append(
                [label.title(), f"{mean_val:.1f}", f"{std_val:.1f}", f"{sem:.2f}"]
            )
        table = ax_bar.table(
            cellText=table_data,
            cellLoc="center",
        ...
        ax_prof.plot(
            vertical_positions,
            vertical_profile,
            label="Vertical (central {}px)".format(central_range),
            color="blue",
        )
        ax_prof.plot(
            horizontal_positions,
            horizontal_profile,
            label="Horizontal (central {}px)".format(central_range),
            color="red",
        )
