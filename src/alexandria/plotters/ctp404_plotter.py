        ...
        **kwargs,
        ...
        if boundary and "x" in boundary and "y" in boundary:
            ...
            _, (boundary_x, boundary_y) = compute_phantom_boundary(
                img, self.analyzer.center, pixel_spacing
            )
        ...
        for i, roi in enumerate(results):
            ...
            radius_mm = roi.get(
                "roi_radius_mm", getattr(self.analyzer, "roi_radius", None)
            )
            radius_px = (
                radius_mm / self.analyzer.pixel_spacing
                if (radius_mm is not None and self.analyzer.pixel_spacing)
                else self.analyzer.roi_radius / self.analyzer.pixel_spacing
            )
        ...
        mean_line = Line2D(
            [0], [0], color="k", linestyle="--", linewidth=1, label="Mean"
        )
        median_line = Line2D(
            [0], [0], color="k", linestyle=":", linewidth=1, label="Median"
        )
        ...
        for i, roi in enumerate(results):
            ...
            if data.size > 0:
                ...
                dist = np.sqrt(
                    (x_indices - radius_px) ** 2 + (y_indices - radius_px) ** 2
                )
