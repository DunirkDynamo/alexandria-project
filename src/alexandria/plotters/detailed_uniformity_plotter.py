...
class DetailedUniformityPlotter:
    ...
    def _get_profiles(self) -> List[Dict[str, Any]]:
        plot_radii = set(
            ...
            for r in getattr(
                self.analyzer,
                "radii_mm",
                self.analyzer.results.get("plot_radii_mm", []),
            )
        )
        if getattr(self.analyzer, "profile_data", None):
            return [
                profile
                for profile in self.analyzer.profile_data
                if round(float(profile.get("radius_mm", 0.0)), 3) in plot_radii
            ]
        profiles = []
    ...
    def plot(self, bins: int = 25, figsize: tuple = (14, 20)) -> plt.Figure:
        ...
        gs = gridspec.GridSpec(
            8, 3, height_ratios=[1.1, 1, 1, 1, 1, 1, 1.1, 0.25], width_ratios=[1, 1, 1]
        )
        ...
        for idx, profile in enumerate(profiles):
            ...
            ax_profile.plot(
                angles, values, color=color, linewidth=1.2, label=f"r={radius:.1f}mm"
            )
            window_length = min(
                17, len(values) - 1 if len(values) % 2 == 0 else len(values)
            )
            if window_length < 5:
                smoothed = values
            else:
                ...
                smoothed = savgol_filter(
                    values, window_length=window_length, polyorder=3, mode="interp"
                )
            ax_smooth.plot(
                angles, smoothed, color=color, linewidth=1.2, label=f"r={radius:.1f}mm"
            )
            mean_val = float(np.mean(values))
            residuals = values - mean_val
            ax_resid.plot(
                angles, residuals, color=color, linewidth=1.0, label=f"r={radius:.1f}mm"
            )
            # Histogram
            ax_hist.hist(
                values,
            ...
            if std_val > 0.0:
                ...
                if x_max > x_min:
                    ...
                    pdf = (1.0 / (std_val * np.sqrt(2.0 * np.pi))) * np.exp(
                        -0.5 * ((x_vals - float(np.mean(values))) / std_val) ** 2
                    )
                    (line,) = ax_fit.plot(
                        x_vals,
        ...
        sg_window = (
            min(
                17,
                (
                    len(profiles[0]["values"]) - 1
                    if len(profiles[0]["values"]) % 2 == 0
                    else len(profiles[0]["values"])
                ),
            )
            if profiles
            else 17
        ...
        sg_poly = 3
        ax_smooth.set_title(
            f"Smoothed Profiles (Savitzky-Golay, window={sg_window}, polyorder={sg_poly})"
        )
        ...
        for idx, profile in enumerate(profiles):
            ...
            ax_rings.plot(
                circle_x, circle_y, color=colors[idx % len(colors)], linewidth=1.2
            )
        ...
        cross_angles_deg_img = (
            -cross_angles_deg_math
        )  # Convert to image convention (clockwise positive)
        ...
        cross_lines = []
        cross_labels = []
        for i, (angle_deg, profile) in enumerate(
            zip(cross_angles_deg_math, cross_profiles)
        ):
            (line,) = ax_cross.plot(
                dists, profile, color=cross_colors[i], label=f"{int(angle_deg)}°"
            )
        ...
        ax_cross_legend.legend(
            cross_lines, cross_labels, loc="center", ncol=4, fontsize=10, frameon=False
        )
        ...
        for i, angle_deg_img in enumerate(cross_angles_deg_img):
            ...
            ax_img.plot(
                ...
                linewidth=2,
                label=f"{int(cross_angles_deg_math[i])}°",
            )
