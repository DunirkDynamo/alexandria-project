...
class HighContrastPlotter:
    ...
    def plot(
        self, figsize: tuple = None, vmin: float = None, vmax: float = None, **kwargs
    ) -> plt.Figure:
        n_profiles = (
            len(self.analyzer.profiles)
            if hasattr(self.analyzer, "profiles") and self.analyzer.profiles
            else 0
        )
        if figsize is None:
        ...
        if (
            hasattr(self.analyzer, "lp_r_mm")
            and hasattr(self.analyzer, "pixel_spacing")
            and self.analyzer.pixel_spacing
        ):
        ...
        if self.analyzer.nMTF is not None and self.analyzer.lp_axis is not None:
            ...
            for threshold, color, label, key in zip(
                mtf_thresholds, colors, labels, threshold_keys
            ):
                ax_mtf.axhline(
                    threshold, color=color, linestyle="--", alpha=0.3, linewidth=1
                )
        ...
        if (
            n_to_show > 0
            and hasattr(self.analyzer, "profiles")
            and self.analyzer.profiles
        ):
            if n_to_show <= 10:
            ...
            for i in range(n_to_show):
                ...
                ax_prof.plot(
                    range(len(profile)), profile, color=colors[i], linewidth=1.5
                )
