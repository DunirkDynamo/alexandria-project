...
class CTP401Plotter:
    ...
    def __init__(self, analyzer, vmin: float = None, vmax: float = None):
        ...
        self.roi_display_names = {
            "LDPE": "ROI0_LDPE",
            "Air": "ROI90_Air",
            "Teflon": "ROI180_Teflon",
            "Acrylic": "ROI270_Acrylic",
        }
    ...
    def plot(self):
        ...
        if boundary and "x" in boundary and "y" in boundary:
            ...
            _, (boundary_x, boundary_y) = compute_phantom_boundary(
                image, self.analyzer.center, pixel_spacing
            )
        ...
        for i, material in enumerate(self.materials):
            ...
            ax_hist = fig.add_subplot(gs[i, 1])
            try:
                mask = self.analyzer._create_circular_mask(
                    ny, nx, center=(cx, cy), radius=radius_px
                )
                data = image[mask]
            except Exception:
                data = np.array([])
