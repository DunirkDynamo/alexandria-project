    ...
    def plot(self):
        ...
        if boundary and "x" in boundary and "y" in boundary:
            ...
            _, (boundary_x, boundary_y) = compute_phantom_boundary(
                display_image, self.center, pixel_spacing
            )
        ...
        for roi_name, roi_data in self.results["blobs"].items():
            ...
            circle = patches.Circle(
                (x, y), r, edgecolor=color, facecolor="none", linewidth=1
            )
            ax_img.add_patch(circle)
            if int(diameter) not in [
                int(label.split("mm")[0]) for label in legend_labels
            ]:
                legend_handles.append(
                    patches.Patch(facecolor="none", edgecolor=color, linewidth=1)
                )
                legend_labels.append(f"{int(diameter)}mm")

        legend_handles.append(
            patches.Patch(
                facecolor="none", edgecolor=bg_color, linewidth=3, linestyle="-"
            )
        )
        ...
        ax_plot.legend(
            lines1 + lines2, labels1 + labels2, loc="lower right", bbox_to_anchor=(1, 0)
        )

        ax_table.axis("off")
        table_data = [["Diameter (mm)", "Mean (HU)", "Std (HU)", "CNR", "Contrast (%)"]]
        roi_list = [
            (float(name.split("_")[1].replace("mm", "")), name, data)
        ...
        table = ax_table.table(
            cellText=table_data, cellLoc="center", loc="center", colWidths=[0.15] * 5
        )
