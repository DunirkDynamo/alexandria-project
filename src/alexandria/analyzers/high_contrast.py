...
class HighContrastAnalyzer:
    ...
    def __init__(
        ...
        self.theta_deg = (
            np.array([10, 40, 62, 85, 103, 121, 140, 157, 173, 186]) + self.t_offset_deg
        )
        self.npeaks = [
            [1, 2],
            [2, 3],
    ...
    def _get_MTF_for_pair(
        ...
        while (len(peaks_max1) < npeaks_expected[1]) or (
            len(peaks_min1) < npeaks_expected[1]
        ):
            if h <= 10:
                return (
                    0.0,
        ...
        for k in range(len(peaks1) - 1):
            if k % 2 == 0:
                tmp_idx = np.array(
                    f1[peaks1[k] - offset : peaks1[k + 1] + offset]
                ).argmax()
        ...
        if (len(Imax) == 0) or (len(Imin) == 0):
            ...
            MTF_value = (np.mean(Imax) - np.mean(Imin)) / (
                np.mean(Imax) + np.mean(Imin)
            )
    ...
    def analyze(self, write_log: bool = False, verbose: bool = True) -> dict:
        ...
        if len(self.per_pair_mtf) > 0:
            self.lp_axis = (
                np.linspace(1, len(self.per_pair_mtf), len(self.per_pair_mtf)) / 10.0
            )
        else:
