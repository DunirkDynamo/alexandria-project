...
class CTP401Analyzer:
    ...
    def analyze(self, t_offset: float = 0.0, verbose: bool = False) -> Dict:
        ...
        for material, angle in self.ROI_CONFIG.items():
            ...
            if verbose:
                print(
                    f"  {material:8s} (angle {angle:4.0f}°): {roi_mean:7.1f} ± {roi_std:5.1f} HU"
                )
        ...
        if abs(denominator) < 1e-6:
            print(
                f"⚠️  Warning: Air and LDPE ROIs have identical means ({roi_results['Air']['mean']:.2f} HU)"
            )
            ...
            lcv = (
                3.25
                * (roi_results["Air"]["std"] + roi_results["LDPE"]["std"])
                / denominator
            )
        ...
        if verbose:
            ...
            print(
                f"Spatial Scaling: X={scale['scaleX_cm']:.2f} cm, Y={scale['scaleY_cm']:.2f} cm"
            )
    ...
    def _compute_spatial_scaling(self, t_offset: float) -> Dict[str, float]:
        ...
        px1 = px[
            max(0, idx_x1 - profile_length) : min(len(px), idx_x1 + profile_length)
        ]
        px2 = px[
            max(0, idx_x2 - profile_length) : min(len(px), idx_x2 + profile_length)
        ]
        py1 = py[
            max(0, idx_y1 - profile_length) : min(len(py), idx_y1 + profile_length)
        ]
        py2 = py[
            max(0, idx_y2 - profile_length) : min(len(py), idx_y2 + profile_length)
        ]
